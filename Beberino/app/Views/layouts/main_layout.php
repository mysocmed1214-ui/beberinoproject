...
<body>
  ...
  <div class="sidebar d-flex flex-column p-3">
    ...
    <a href="<?= site_url('admin/activity-logs') ?>" class="<?= current_url() == site_url('admin/activity-logs') ? 'active' : '' ?>">
...
<div class="mt-3 px-3">
    <?php
        ...
        $systemMode = $settingsModel->getValue('system_mode') ?? 'online';
    ?>
    <div class="form-check form-switch">
        <input class="form-check-input" type="checkbox" id="systemModeSwitchSidebar" <?= $systemMode === 'online' ? 'checked' : '' ?>>
        <label class="form-check-label text-white" for="systemModeSwitchSidebar">
            <?= $systemMode === 'online' ? 'System Online' : 'Maintenance' ?>
        </label>
    </div>
</div>
    <hr>
  ...
  <script>
document.getElementById('systemModeSwitchSidebar').addEventListener('change', function() {
    ...
    .then(data => {
        if(data.status === 'success') {
            const label = document.querySelector('label[for="systemModeSwitchSidebar"]');
            label.textContent = data.newMode === 'online' ? 'System Online' : 'Maintenance';
        } else {
